-- Store Service Coverage Materialized View (CORRECT SCHEMA)
...
SELECT 
  ...
  sapp.area_name,
  sapp.product_name as product_type,
  sapp.price as price_basic,  -- Column is 'price' not 'price_basic'
  -- Computed field for quick filtering
  s.status = 'active' AND s.is_open = true AND sapp.is_available = true as is_available
FROM stores s
