...
import { ChevronLeft, ChevronRight, Sparkles, Heart, Flower } from 'lucide-react'
  ...
  if (isDesktop) {
    return (
      <section className="relative overflow-hidden bg-white">
        ...
        <Link 
          href="/points-guide" 
          className="fixed bottom-10 right-10 z-50 bg-rose-500 hover:bg-rose-600 transition-all duration-300 rounded-full w-16 h-16 flex items-center justify-center shadow-xl hover:shadow-2xl group animate-bounce"
        >
          <span className="text-white text-2xl font-bold">P</span>
      ...
      <div className="px-4 py-6 bg-white">
        ...
        <div className="flex gap-2 justify-center">
          {categories.map((category) => {
            const IconComponent = category.icon
            return (
              <Link key={category.id} href={`/category/${category.id}`} className="flex-1 max-w-[120px]">
                <div className="bg-white border border-neutral-200 rounded-lg p-3 hover:border-rose-300 hover:shadow-md transition-all">
                  <div className={`w-10 h-10 rounded-full ${category.iconBg} flex items-center justify-center mx-auto mb-2`}>
