...
-- 9. 테스트용 데이터 업데이트 (개발 환경용)
-- 일부 주문을 긴급 상태로 만들기
DO $$
BEGIN
    -- Critical 긴급도 테스트 데이터
    UPDATE orders 
    SET 
...
AND column_name IN ('accepted_at', 'urgency_level', 'contact_attempts')
...
WHERE routine_name IN ('update_accepted_at', 'add_contact_attempt')

UNION ALL

SELECT 
    'Urgent orders' as check_type,
    COUNT(*) as count
FROM orders
WHERE status = 'accepted' 
AND urgency_level IN ('medium', 'high', 'critical');