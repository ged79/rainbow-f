...
export default function FuneralPage() {
  ...
  const loadProducts = async () => {
    try {
      setLoading(true)
      const dbProducts = await getProductsByCategoryGrouped('funeral')
        ...
        <div className="max-w-7xl mx-auto px-4 pt-6 pb-4 md:pt-20 md:pb-10">
          <div className="text-center">
            ...
            <p className="text-sm md:text-lg text-gray-600 max-w-2xl mx-auto">
              <span className="md:hidden">깊은 애도와 위로를 전합니다</span>
              <span className="hidden md:block">고인의 명복을 빌며 깊은 애도와 위로의 마음을 전합니다</span>
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="flex gap-6 py-4 overflow-x-auto scrollbar-hide">
              {categoryOrder.map((subCategory) => (
                <a
                  key={subCategory}
                  href={`#${subCategory}`}
                  className="flex items-center gap-1 text-sm font-medium text-gray-700 hover:text-gray-900 whitespace-nowrap transition-colors"
                >
        ...
        {categoryOrder.length === 0 ? (
          <div className="text-center py-16">
            <div className="w-20 h-20 rounded-full bg-gray-100 flex items-center justify-center mb-4 mx-auto">
              <Flower className="w-10 h-10 text-gray-400" />
            </div>
              ...
              <div className="flex items-center justify-between mb-8">
                <div>
                  ...
                  <p className="text-gray-600">
                    {subCategory === '근조화환' && '고인을 추모하는 정중한 근조화환'}
                    {subCategory === '근조장구' && '품격있는 근조 스탠드'}
                    {subCategory === '근조꽃바구니' && '애도의 마음을 담은 꽃바구니'}
                  </p>
                </div>
